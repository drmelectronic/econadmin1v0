...
class Ventana(gtk.Window):
  ...
  def enviar(self,widget):
    ...
    enviado = self.model[int(path[0])][6]
    if enviado:
      body = 'Saludos cordiales,\nLe reenviamos nuestra cotización por <b>%s</b>.\nAtte'%nombre
    else:
...
class Nueva(gtk.Dialog):
	...
	def set_id(self):
	  self.cursor.execute("SELECT CORR FROM cotizaciones WHERE YEAR(FECHA)=%d ORDER BY YEAR(FECHA) DESC, CORR DESC LIMIT 1"%self.but_fecha.get_date().year)
	  try:
	    corr = self.cursor.fetchone()[0]+1
	  except:
	    corr = 1
